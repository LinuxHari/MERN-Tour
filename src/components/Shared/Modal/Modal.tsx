...
const Trigger = ({ children, className = "" }: ModalProps) => {
  ...
  return (
    <Button type="button" buttonType="primary" className={`${isOpen? "d-none": "d-inline-block" } ${className}`} onClick={onOpen} showIcon={false}>
      {children}
    </Button>
  )
};
...
const Header = ({ children, className = "" }: ModalProps) => <div className={`border-1-bottom py-2 ${className}`}>{children}</div>;

const Title = ({ children, className = "" }: ModalProps) => <h5 className={`${className}`}>{children}</h5>;

const Content = ({ children, className = "" }: ModalProps) => <div className={`border-1-bottom py-2 ${className}`}>{children}</div>;
