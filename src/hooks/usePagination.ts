...
const usePagination = (page: number, fn: PaginationFn, count: number) => {
  ...
  useLayoutEffect(() => {
    if(numOfPages < buttonsToShow){
      ...
      const pages = [1, null, ...Array.from({length: buttonsToShow}).map((_, index) => numOfPages - buttonsToShow + index + 1)]
      setPages(pages)
    }
    else{
      const pages = [1, null, ...Array.from({length: buttonsToShow - 2}).map((_, index) => page - 1 + index), null, numOfPages]
      setPages(pages)
    }
  }, [page, width])
