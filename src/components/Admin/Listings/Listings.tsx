...
const Listings = ({render}: RenderProps) => {
  ...
  return (
    ...
    {
      tourData.map(({imageSrc, location, title, rating, reviewCount, duration, price}) => (
        <ListingCard img={imageSrc} location={location} title={title} rating={rating} reviewCount={reviewCount} duration={duration} price={price}/>
      ))
    }
    </div>
    <div className="mt-30">
